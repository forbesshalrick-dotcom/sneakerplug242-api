...
async function runChat(req, sub, userText, token, ctx = {}, image = null) {
  ...
  if (staffName && image) {
    ...
    const floatLine = floatShould != null
      ? (' 💡 Tonight the float SHOULD hold $' + floatShould.toFixed(2) + '. Compare your count: (a) if it EXACTLY MATCHES $' + floatShould.toFixed(2) + ', do NOT ask them to confirm — call record_float_count RIGHT NOW and tell them it matches, it is logged, and they are closed out ✅. (b) if it is SHORT (less than $' + floatShould.toFixed(2) + '), say plainly it is short by the difference and to recount/straighten it before closing — do NOT log yet. (c) if it is OVER, tell them it is over by the difference and ask them to recount once (an over is usually a miscount or an unlogged sale); log it once they confirm the recount.')
