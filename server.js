...
async function runChat(req, sub, userText, token, ctx = {}, image = null) {
  ...
  try {
    ...
    if (FIT_QUESTION_RE.test(String(userText || ''))) {
      system += `\n\n👟 THEY JUST ASKED ABOUT FIT. Do NOT send them to friends, reviews or Google, and do NOT explain that sneakers vary. In Nassau we bring BOTH sizes to the door, they try both on and keep the one that fits — say that, confidently, then ask their size. (Island shipments are pay-first with no try-on: there, tell them the sizes that shoe comes in and help them pick.)`;
    }
