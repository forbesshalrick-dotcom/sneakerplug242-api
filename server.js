...
async function runChat(req, sub, userText, token, ctx = {}, image = null) {
  ...
  if (staffName && image) {
    ...
    const floatLine = floatShould != null
      ? (' 💡 Tonight the float SHOULD hold $' + floatShould.toFixed(2) + '. 📸 MULTIPLE PHOTOS: the float may not fit in one shot, so keep a RUNNING TOTAL across every cash photo they send in this chat that has not been logged yet — ADD each new photo to the earlier ones, never restart the count. Then: (a) if the running total EXACTLY MATCHES $' + floatShould.toFixed(2) + ', call record_float_count RIGHT NOW and tell them it matches, it is logged, they are closed out ✅. (b) if the running total is UNDER $' + floatShould.toFixed(2) + ', do NOT call it short yet — say "I have got $X so far — is that all the cash, or is there more? send another photo if so 📸". ONLY once they confirm that is ALL the cash is it truly SHORT (then say short by the difference, recount/straighten before closing, do not log). (c) if the running total is OVER, ask them to recount once (usually a miscount or an unlogged sale); log once they confirm.')
      : ' Keep a running total if they send more than one cash photo (add them up, do not restart). Show your total and ask "that right?", then call record_float_count once they confirm.';
    userText = '(SYSTEM NOTE — this is NOT the person\'s words, do NOT quote it or say "I got your note": the photo in this message is from ' + staffName + ', one of OUR STAFF. It is their end-of-shift FLOAT CASH or a RECEIPT — never a shoe. If it is cash/banknotes: count it denomination by denomination — Bahamian $ and US $ each 1:1. ⚠️ COUNTING ACCURACY: if the bills OVERLAP, are folded, stacked, or you cannot clearly see every single note, do NOT guess — ask them to spread the cash out FLAT on the table with no overlapping and send one more photo (2026-07-17: Kiki kept counting $60 as $50 because bills were stacked). Once you can count cleanly, show your count.' + floatLine + ' If it is a receipt: read the total and call record_expense. Keep replies short — nothing about shoes, sizes, or "if it was a shoe photo".)'
...
• 🎽 STAFF EXCEPTION — CHECK THIS FIRST: if the prompt has a "STAFF CHAT" section (i.e. this person is one of OUR staff, recognized by their number) AND the photo is CASH / banknotes / a pile of money, it is their END-OF-SHIFT FLOAT COUNT — go STRAIGHT to the float-count flow (count it denomination by denomination and call record_float_count per the staff rules). Do NOT call it a "banknote sent by accident", do NOT say "not a shoe", and NEVER pitch sneakers or ask a staff member their size (2026-07-17: a staff float photo on OSC got "that's cash, want to see what we've got? tell me your size" — staff were detected but Kiki still ran the customer script). The rest of THIS rule is for CUSTOMERS only.
